...
After fixing those it reports 23. A third pass over block plugins, two SEO plugins and Elementor Pro (1,919 more files) removed three further classes: REST route arguments passed as a variable rather than an inline array, a variable re-assigned through a sanitizer before being echoed, and `$wpdb` calls whose SQL was prepared on the preceding line. Elementor Pro and All in One SEO now scan with **zero criticals** across 830 and 598 files, and Yoast Premium with zero across 276 — which is the result a scanner should produce on well-maintained code. The lesson generalises: **a scanner calibrated on one class of code fails silently on another.** Amateur theme code has no query builders, no variadic wrappers and no annotations, so none of those patterns appear there.
